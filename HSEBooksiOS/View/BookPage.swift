...
struct BookPage: View {
    ...
    private var genres: String {
        ...
        return "Genre: \(genresStrings.joined(separator: ", "))"
    }
...
//        guard let credentials = appContext.credentials else { return }
//        viewState = .loading
        // /user/1/wishlist?baseId=1
//        RequestService.shared.makePostRequest(to: "\(User.user.endpoint)/\(credentials.user.id)/wishlist", with: credentials.token, body: String(), params: ["baseId" : "\(bookBase.id)"]) { (result: Result<Decodable, RequestError>) in
//
//        }
    }
...
struct BookPage_Previews: PreviewProvider {
    static var previews: some View {
        BookPage(bookBase: .constant(BookBase.getItems(amount: 1).first!))
    }
}
